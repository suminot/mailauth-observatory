...
DESIGN.md 第8章の Sprint 1〜10 は実装済みで、第9章の受け入れ基準は skip 1件を
除いて機械検査している。**ただし「実装側の残件が無い」わけではない** ──
実測して分かった改善点は `BACKLOG.md` に集めてある。この文書は運営者が
手を動かす作業、BACKLOG は実装側の作業である。
...
- 3,830社 ÷ 0.5 = **最低約2.1時間**（qps だけから出した下限）
- **実測はその4倍かかっている。** 2026-09 の実行で1件あたり16.3秒、
  新規1,005件で4時間21分。crt.sh は Actions の IP 帯に対して遅く、
  応答1本あたり約65秒かかっている
- 大企業は SAN が数千件返る（`fastretailing.com` で 5,161 件）ので実際はさらに伸びる
- **GitHub Actions の1ジョブ上限は6時間**（このワークフローは5時間30分で切る）
