# Email DNS Monitor（メール認証月次計測システム） 設計仕様書
...
| 実行時間 | **目標** 3時間以内（GitHub Actions のジョブ上限6時間）。実測は `runs/<月>.md` を見ること ── 達成 qps は実行ごとに変わる |
| 実行頻度 | 月次（当初はアドホック手動） |
| 月額コスト目標 | 0円 |
...
クエリ数はこの通りだが、**実時間は達成 qps 次第で、そこが一定しない。**
設定の `rate.qps` は上限であって速度ではない。

全ドメインにフル計測をかけた場合は 30,000 × 59 = 1,770,000 クエリとなり、
階層分割の3倍になる。階層分割は必須である。
```

**達成 qps は実行のたびに変わる。文書に固定の時間を書かない。**
2026-09 の国内計測では、1ドメインを1本ずつ直列に引いていたため上流への
往復がそのまま積み上がり、設定の 80 qps には一度も届かなかった。
その後 P4 と P3 を応答待ちだけ重ねる形にしてある（`rate.concurrency`）。

**並行化後の実地の数字はまだ無い。** 見積りではなく実測を見ること ──
各月の `runs/<月>.md` と `breakdown.resolver` が正本である。
...
| バックエンド | 用途 | 備考 |
|---|---|---|
| `zdns` | 任意（未導入でも動く） | Apache-2.0、JSON Lines で status/answers/TTL/authorities/protocol を生保存。MX/TXT/SPF/DMARC 専用モジュールあり |
| `dnsx` | 補助・クロスチェック | MIT、`-raw` で生レスポンス |
| `dnspython` | **既定** | 例外分岐が細かく制御しやすい。`--method` を指定しなければこれが動く |
...
**既定は `dnspython`。** zdns は入っていなくても計測は回る。
...
- **完了条件**: 失敗率2%未満、30,000ドメインで実行時間3時間以内、bronze が JSON Lines で保存される
  - **この規模での実測はまだ無い**（2026-09 の国内計測は 2,153 ドメイン）。達成 qps が変われば結論も変わるので、`runs/<月>.md` の実測で判断すること
...
```
このリポジトリで、Email DNS Monitor（メール認証月次計測システム）を実装します。
